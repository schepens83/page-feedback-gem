...
RSpec.describe "page-feedback.html" do
  ...
  it "is a self-contained Phase 0 report" do
    expect(document.at_css("title").text).to include("PageFeedback")
    expect(document.css("script[src], link[rel='stylesheet']")).to be_empty
    expect(document.text).to include("Phase 0", "4 examples, 0 failures")
  end
